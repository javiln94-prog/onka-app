 ...
 * Guarda los datos en data/data.json.
 *
...
// -----------------------------------------------------------------
// Datos iniciales (solo se usan la primera vez, si no existe data.json)
// -----------------------------------------------------------------
function seedData() {
  return {
...
// -----------------------------------------------------------------
// Almacenamiento con cola de escritura
// -----------------------------------------------------------------
...
function loadSync() {
  ...
  if (!fs.existsSync(DATA_FILE)) {
    ...
    // Migración suave: asegurar que existen todos los campos nuevos
    if (!cache.responsablesAuxiliar) cache.responsablesAuxiliar = [];
    if (!cache.responsablesProyecto) cache.responsablesProyecto = [];
...
async function handleApi(req, res, pathname, query) {
  ...
  if (pathname === "/api/users" && req.method === "GET") {
    ...
    return sendJSON(res, 200, loadSync().users);
  }
  ...
  // ---- Calendario ----
  if (pathname === "/api/events" && req.method === "GET") return sendJSON(res, 200, loadSync().events);
